    ...
    <AnimatePresence>
        {showAbout && (
          <motion.div variants={boxVariants}
            ...
            animate="visible"
            exit="hidden"
            transition={{ duration: 0.5, ease: 'easeInOut' }} className="padding-x z-20 w-full absolute py-12 bg-white flex max-lg:hidden">
            {AboutData.map(data => {
              return (
                <div className="w-1/3 min-w-2xl" key={data.heading}>
                  <h1 className=" text-teal-600 text-lg mb-7 font-medium">{data.heading}</h1>
                  <div className="flex flex-col gap-7 text-xl font-medium text-gray-700">
                    {(data.links).map(link => {
                      return (
